...
#         self.word = input('문장 입력: ')
    ...
    def dao_split(self):
        ...
        print('ac_re')
        print(self.ac_result)
        print('ob_re')
        print(self.ob_result)
        if self.ob_result is None and self.ac_result is None:
            self.em_result = self.Kobert(self.word)
            self.word_em = self.dao.sel_emotion(self.em_result)
            self.word_em = self.word_em.replace('[','').replace(']','').replace(',','').replace(' ', ',').split(',')
    ...
    def fin(self):
        if isinstance(self.final, list):
            ...
            for i in self.final:
                return self.dao.sel_main(i)
#         else:
#                 return self.dao.sel_emotion(self.final)